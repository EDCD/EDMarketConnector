...
class WinConfig(AbstractConfig):
    ...
    def __init__(self, do_winsparkle=True) -> None:
        super().__init__()
        self.app_dir_path = (
            pathlib.Path(known_folder_path(FOLDERID_LocalAppData)) / appname
        )
        ...
        journal_dir_path = (
            pathlib.Path(known_folder_path(FOLDERID_SavedGames))
            / "Frontier Developments"
            / "Elite Dangerous"
        )
        self.default_journal_dir_path = (
            journal_dir_path if journal_dir_path.is_dir() else None
        )
        ...
        self.set(
            "outdir",
            docs_path
            if docs_path is not None and pathlib.Path(outdir_str).is_dir()
            else self.home,
        )
    ...
    def set(self, key: str, val: Union[int, str, List[str], bool]) -> None:
        ...
        if isinstance(val, str):
            ...
            winreg.SetValueEx(
                self.__reg_handle, key, REG_RESERVED_ALWAYS_ZERO, reg_type, val
            )

        elif isinstance(val, int):
            reg_type = winreg.REG_DWORD
            winreg.SetValueEx(
                self.__reg_handle, key, REG_RESERVED_ALWAYS_ZERO, reg_type, int(val)
            )
