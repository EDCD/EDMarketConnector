...
class LoggingHandler(server.BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 # I cant change it
        ...
        data_raw: bytes = self.rfile.read(content_length)

        encoding = self.headers.get("Content-Encoding")
        data = self.get_printable(data_raw, encoding)
        to_save = data

        if len(self.path) > 1 and self.path.startswith("/"):
            target_path = self.path[1:]
        elif self.path == "/":
            target_path = "WEB_ROOT"
        else:
            target_path = self.path

        response_handler = DEFAULT_RESPONSES.get(target_path)
        response = (response_handler(to_save) if callable(response_handler) else response_handler)
        ...
        target_file = output_data_path / f"{safe_file_name(target_path)}.log"
        ...
        with output_lock, target_file.open("a", encoding="utf-8") as file:
            file.write(f"{to_save}\n\n")

    @staticmethod
    def get_printable(data: bytes, compression: Literal["deflate", "gzip"] | str | None = None) -> str:
        """Convert an incoming compressed or raw data stream into a string.
...
def safe_file_name(name: str) -> str:
    """Escape special characters out of a file name safely.
...
def extract_edsm_data(data: str) -> dict[str, Any]:
    ...
    res = parse_qs(data)
    return {name: values[0] for name, values in res.items() if values}
...
def generate_edsm_response(raw_data: str) -> str:
    ...
    try:
        ...
        logger.exception("Failed to parse EDSM debug request envelope payload")
        return "UNKNOWN REQUEST"

    out = {
        "msgnum": 100,  # OK Status Code
        "msg": "debug stuff",
        "events": [
            (
                {
                ...
                else {"event": "unknown", "msgnum": 100, "msg": "debug stuff"}
            )
            for e in events
        ],
    }
...
DEFAULT_RESPONSES: dict[str, Callable[[str], str]] = {
