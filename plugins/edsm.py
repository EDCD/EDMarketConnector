...
def credentials(cmdr: str) -> Optional[Tuple[str, str]]:
    ...
    if (
        cmdr in cmdrs
        and (edsm_usernames := config.get_list("edsm_usernames"))
        ...
        logger.trace_if(
            CMDR_EVENTS,
            f"{cmdr=}: returning ({edsm_usernames[idx]=}, {edsm_apikeys[idx]=})",
        )
...
def get_discarded_events_list() -> None:
    ...
    try:
        ...
        logger.warning(
            "Exception whilst trying to set this.discarded_events:", exc_info=e
        )
    ...
    while True:
        ...
        while retrying < 3:
            should_skip, new_item = killswitch.check_killswitch(
                ...
                item
                if item is not None
                else cast(Tuple[str, Mapping[str, Any]], ("", {})),
                logger,
            )
            ...
            try:
                ...
                if pending and should_send(pending, entry["event"]):
                    ...
                    logger.trace_if(
                        "plugin.edsm.api", f"API response content: {r.content!r}"
                    )
                    r.raise_for_status()
...
def should_send(entries: List[Mapping[str, Any]], event: str) -> bool:  # noqa: CCR001
    ...
    for entry in entries:
        if (entry["event"] == "Cargo" and not this.newgame_docked) or entry[
            "event"
        ] == "Docked":
            ...
            "ModuleBuy",
            "ModuleSell",
            "ModuleSwap",  # will be shortly followed by "Loadout"
            "ShipyardBuy",
            "ShipyardNew",
            "ShipyardSwap",
        ):  # "
