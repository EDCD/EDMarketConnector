...
class This:
    ...
    def __init__(self):
        ...
        # Game state
        self.multicrew: bool = (
            False  # don't send captain's ship info to EDSM while on a crew
        )
...
def system_url(system_name: str) -> str:
    ...
    if this.system_address:
        return requests.utils.requote_uri(
            f"https://www.edsm.net/en/system?systemID64={this.system_address}"
        )

    if system_name:
        return requests.utils.requote_uri(
            f"https://www.edsm.net/en/system?systemName={system_name}"
        )
...
def plugin_start3(plugin_dir: str) -> str:
    ...
    if not config.get_list("edsm_cmdrs"):
        if (
            isinstance(config.get_list("cmdrs"), list)
            and config.get_list("edsm_usernames")
            and config.get_list("edsm_apikeys")
        ):
...
def journal_entry(  # noqa: C901, CCR001
    ...
    should_return, new_entry = killswitch.check_killswitch(
        "plugins.edsm.journal", entry, logger
    )
    if should_return:
...
entry: {entry!r}""",
    ...
    if config.get_str("station_provider") == "EDSM":
        ...
        if this.station_link:
            ...
            this.station_link["url"] = station_url(
                str(this.system_name), str(this.station_name)
            )
    ...
    if (
        ...
        if not monitor.monitor.is_live_galaxy():
            ...
            if this.legacy_galaxy_last_notified is None or (
                datetime.now(timezone.utc) - this.legacy_galaxy_last_notified
        ...
        if entry["event"] == "LoadGame":
            ...
            materials = {
                ...
                "Manufactured": [
                    {"Name": k, "Count": v} for k, v in state["Manufactured"].items()
                ],
                "Encoded": [
                    {"Name": k, "Count": v} for k, v in state["Encoded"].items()
                ],
            }
            materials.update(transient)
            logger.trace_if(
                CMDR_EVENTS, f'"LoadGame" event, queueing Materials: {cmdr=}'
            )
...
def get_discarded_events_list() -> None:
    ...
    try:
        r = this.session.get(
            "https://www.edsm.net/api-journal-v1/discard", timeout=_TIMEOUT
        )
        r.raise_for_status()
        ...
        logger.warning(
            "Exception while trying to set this.discarded_events:", exc_info=e
        )
...
def worker() -> None:  # noqa: CCR001 C901
    ...
    while not this.discarded_events:
        if this.shutting_down:
            logger.debug(
                f"returning from discarded_events loop due to {this.shutting_down=}"
            )
            return
    ...
    while True:
        ...
        if item:
            ...
            logger.trace_if(
                CMDR_EVENTS,
                f'De-queued ({cmdr=}, {game_version=}, {game_build=}, {entry["event"]=})',
            )
        else:
        ...
        while retrying < 3:
            ...
            should_skip, new_item = killswitch.check_killswitch(
                "plugins.edsm.worker", item, logger
            )
            ...
            try:
                if item and entry["event"] not in this.discarded_events:
                    logger.trace_if(
                        CMDR_EVENTS,
                        f'({cmdr=}, {entry["event"]=}): not in discarded_events, appending to pending',
                    )
                    ...
                    if (
                        ...
                        or last_game_version != game_version
                        or last_game_build != game_build
                    ):
                        pending = []
                ...
                new_pending = []
                for e in pending:
                    skip, new = killswitch.check_killswitch(
                        f'plugin.edsm.worker.{e["event"]}', e, logger
                    )
                    if skip:
                        continue
                ...
                if pending and should_send(pending, entry["event"]):
                    logger.trace_if(
                        CMDR_EVENTS,
                        f'({cmdr=}, {entry["event"]=}): should_send() said True',
                    )
                    logger.trace_if(
                        CMDR_EVENTS,
                    ...
                    if any(
                        ...
                        for p in pending
                        if p["event"]
                        in ("CarrierJump", "FSDJump", "Location", "Docked")
                    ):
                        logger.trace_if(
                            "journal.locations",
                    ...
                    logger.trace_if(
                        CMDR_EVENTS,
                        f'({cmdr=}, {entry["event"]=}): Using {username=} from credentials()',
                    )
                    ...
                    data = {
                        ...
                        "message": json.dumps(pending, ensure_ascii=False).encode(
                            "utf-8"
                        ),
                    }
                    ...
                    if any(
                        ...
                        if isinstance(data_elided["message"], bytes):
                            data_elided["message"] = data_elided["message"].decode(
                                "utf-8"
                            )
                        if isinstance(data_elided["commanderName"], bytes):
                            data_elided["commanderName"] = data_elided[
                                "commanderName"
                            ].decode("utf-8")
                        logger.trace_if(
                            "journal.locations",
                    ...
                    response = this.session.post(
                        TARGET_URL, data=data, timeout=_TIMEOUT
                    )
                    logger.trace_if(
                        "plugin.edsm.api", f"API response content: {response.content!r}"
                    )
                    ...
                    if msg_num // 100 == 2:
                        ...
                        for e, r in zip(pending, reply["events"]):
                            if not closing and e["event"] in (
                                ...
                                if (
                                    not config.shutting_down
                                    and this.system_link is not None
                                ):
                ...
                logger.debug(
                    f"Attempt to send API events: retrying == {retrying}", exc_info=e
                )
                retrying += 1
        ...
        if entry["event"].lower() in ("shutdown", "commander", "fileheader"):
            ...
            pending = []
            logger.trace_if(
                CMDR_EVENTS, f'Blanked pending because of event: {entry["event"]}'
            )
        if closing:
...
def should_send(entries: List[Mapping[str, Any]], event: str) -> bool:  # noqa: CCR001
    ...
    if this.navbeaconscan:
        if entries and entries[-1]["event"] == "Scan":
            ...
            logger.trace_if(
                CMDR_EVENTS,
                f"False because {this.navbeaconscan=}"
    ...
    logger.trace_if(
        CMDR_EVENTS,
        f"False as default: {this.newgame_docked=}" if not should_send_result else "",
    )
    return should_send_result
