...
def system_url(system_name: str) -> str:
    if this.system_address:
        return requests.utils.requote_uri(
            f"https://eddb.io/system/ed-address/{this.system_address}"
        )
...
def station_url(system_name: str, station_name: str) -> str:
    if this.station_marketid:
        return requests.utils.requote_uri(
            f"https://eddb.io/station/market-id/{this.station_marketid}"
        )
...
def journal_entry(cmdr, is_beta, system, station, entry, state):
    ...
    should_return, new_entry = killswitch.check_killswitch(
        f'plugins.eddb.journal.event.{entry["event"]}', new_entry
    )
    if should_return:
        return
