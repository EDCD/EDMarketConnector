...
FAKE = (
    ...
    "Training",
    "Destination",
)  # Fake systems that shouldn't be sent to Inara
...
class This:
    ...
    def __init__(self):
        ...
        self.lastship = (
            None  # eventData from the last addCommanderShip or setCommanderShip event
        )
        ...
        self.multicrew: bool = (
            False  # don't send captain's ship info to Inara while on a crew
        )
        ...
        self.event_lock: Lock = (
            threading.Lock()
        )  # protects events, for use when rewriting events

    def filter_events(
        self, key: Credentials, predicate: Callable[[Event], bool]
    ) -> None:
        """
...
EVENT_COLLECT_TIME = (
    31  # Minimum time to take collecting events before requesting a send
)
...
def system_url(system_name: str) -> str:
    ...
    if this.system_address:
        return requests.utils.requote_uri(
            f"https://inara.cz/galaxy-starsystem/" f"?search={this.system_address}"
        )

    if system_name:
        return requests.utils.requote_uri(
            f"https://inara.cz/galaxy-starsystem/" f"?search={system_name}"
        )
...
def plugin_prefs(parent: ttk.Notebook, cmdr: str, is_beta: bool) -> tk.Frame:
    ...
    HyperlinkLabel(
        ...
        columnspan=2, padx=x_padding, sticky=tk.W
    )  # Don't translate
    ...
    this.log_button = nb.Checkbutton(
        frame,
        text=_(
            "Send flight log and Cmdr status to Inara"
        ),  # LANG: Checkbox to enable INARA API Usage
        variable=this.log,
    ...
    this.label = HyperlinkLabel(
        ...
        text=_(
            "Inara credentials"
        ),  # LANG: Text for INARA API keys link ( goes to https://inara.cz/settings-api )
        background=nb.Label().cget("background"),
...
def journal_entry(  # noqa: C901, CCR001
    ...
    should_return, new_entry = killswitch.check_killswitch(
        "plugins.inara.journal", entry, logger
    )
    if should_return:
        plug.show_error(
            _("Inara disabled. See Log.")
        )  # LANG: INARA support disabled via killswitch
    ...
    if not monitor.is_live_galaxy():
        ...
        if (
            (
                ...
                or (datetime.now(timezone.utc) - this.legacy_galaxy_last_notified)
                > timedelta(seconds=300)
            )
            and config.get_int("inara_out")
            and not (is_beta or this.multicrew or credentials(cmdr))
        ):
    ...
    elif event_name in ("ShipyardNew", "ShipyardSwap") or (
        event_name == "Location" and entry["Docked"]
    ):
        this.suppress_docked = True

    if (
        config.get_int("inara_out")
        and not is_beta
        and not this.multicrew
        and credentials(cmdr)
    ):
        current_credentials = Credentials(
            this.cmdr, this.FID, str(credentials(this.cmdr))
        )
        try:
            if (
                this.newuser
                or event_name == "StartUp"
                or (this.newsession and event_name == "Cargo")
            ):
                this.newuser = False
                this.newsession = False
                ...
                if state["Engineers"]:
                    engineer_data = [
                        {
                            "engineerName": k,
                            "rankValue": v[0] if isinstance(v, tuple) else None,
                            "rankStage": v,
                        }
                        for k, v in state["Engineers"].items()
                    ]
                    new_add_event(
                        "setCommanderRankEngineer", entry["timestamp"], engineer_data
                    )
                ...
                if state["ShipID"]:
                    ...
                    new_add_event(
                        "setCommanderShipLoadout", entry["timestamp"], this.loadout
                    )
                ...
                new_add_event(
                    "setCommanderRankEngineer", entry["timestamp"], engineer_rank_data
                )
            ...
            # Location change
                ...
                if this.undocked:
                    ...
                    new_add_event("addCommanderTravelDock", entry["timestamp"], to_send)
                ...
                if system:
                    ...
                    new_add_event(
                        "setCommanderTravelLocation", entry["timestamp"], to_send
                    )
                ...
                new_add_event("addCommanderTravelFSDJump", entry["timestamp"], to_send)
                ...
                new_add_event(
                    "addCommanderTravelCarrierJump", entry["timestamp"], to_send
                )
            ...
            cargo = [
                OrderedDict({"itemName": k, "itemCount": state["Cargo"][k]})
            ...
            if this.materials != materials:
                new_add_event(
                    "setCommanderInventoryMaterials", entry["timestamp"], materials
                )
        ...
        if event_name == "LoadGame":
            ...
            if entry.get("Bank_Account") is not None:
                if entry["Bank_Account"].get("Current_Wealth") is not None:
                    inara_data["commanderAssets"] = entry["Bank_Account"][
                        "Current_Wealth"
                    ]

            new_add_event("setCommanderCredits", entry["timestamp"], inara_data)
            new_add_event(
                "setCommanderGameStatistics", entry["timestamp"], state["Statistics"]
            )  # may be out of date
        ...
        if event_name == "StoredShips":
                    ...
                    OrderedDict(
                        {
                            ...
                            "starsystemName": x.get(
                                "StarSystem"
                            ),  # Not present for ships in transit
            ...
            if this.fleet != fleet:
                this.fleet = fleet
                this.filter_events(
                    current_credentials, lambda e: e.name != "setCommanderShip"
                )
            ...
            if this.loadout != loadout:
                ...
                this.filter_events(
                    ...
                    lambda e: (
                        ...
                        or cast(dict, e.data)["shipGameID"]
                        != cast(dict, this.loadout)["shipGameID"]
                    ),
                )
        ...
        if event_name == "StoredModules":
            items = {
                mod["StorageSlot"]: mod for mod in entry["Items"]
            }  # Impose an order
            ...
            if this.storedmodules != modules:
                ...
                this.filter_events(
                    current_credentials,
                    lambda e: e.name != "setCommanderStorageModules",
                )

                # this.events = list(filter(lambda e: e['eventName'] != 'setCommanderStorageModules', this.events))
                new_add_event(
                    "setCommanderStorageModules", entry["timestamp"], this.storedmodules
                )
        ...
        if event_name == "MissionAccepted":
            ...
            for x in entry.get("PermitsAwarded", []):
                new_add_event(
                    "addCommanderPermit", entry["timestamp"], {"starsystemName": x}
                )
            ...
            if "PermitsAwarded" in entry:
                data["rewardPermits"] = [
                    {"starsystemName": x} for x in entry["PermitsAwarded"]
                ]

            if "CommodityReward" in entry:
                data["rewardCommodities"] = [
                    {"itemName": x["Name"], "itemCount": x["Count"]}
            ...
            for faction in entry.get("FactionEffects", []):
                effect: OrderedDictT[str, Any] = OrderedDict(
        ...
        if event_name == "Died":
            ...
            if data["opponentName"] == "":
                logger.warning(
                    'Dropping addCommanderCombatInterdicted message because opponentName came out as ""'
                )
            ...
            if data["opponentName"] == "":
                logger.warning(
                    'Dropping addCommanderCombatInterdiction message because opponentName came out as ""'
                )

            else:
                new_add_event(
                    "addCommanderCombatInterdiction", entry["timestamp"], data
                )
        ...
        # New Odyssey features
            ...
            to_send_data: Optional[
                Dict[str, Any]
            ] = {}  # This is a glorified sentinel until lower down.
            # On Horizons, neither of these exist on TouchDown
            star_system_name = entry.get("StarSystem", this.system_name)
            body_name = entry.get(
                "Body", state["Body"] if state["BodyType"] == "Planet" else None
            )

            if star_system_name is None:
                logger.warning(
                    "Refusing to update addCommanderTravelLand as we dont have a StarSystem!"
                )
                to_send_data = None

            if body_name is None:
                logger.warning(
                    "Refusing to update addCommanderTravelLand as we dont have a Body!"
                )
                to_send_data = None

            if (op := entry.get("OnPlanet")) is not None and not op:
                logger.warning(
                    "Refusing to update addCommanderTravelLand when OnPlanet is False!"
                )
            ...
            if not entry["PlayerControlled"]:
                logger.info(
                    "Not updating inara addCommanderTravelLand for autonomous recall landing"
                )
                to_send_data = None
            ...
            if to_send_data is not None:
                ...
                to_send_data["starsystemBodyCoords"] = [
                    entry["Latitude"],
                    entry["Longitude"],
                ]
                ...
                new_add_event(
                    "addCommanderTravelLand", entry["timestamp"], to_send_data
                )
            ...
            odyssey_plural_microresource_types = (
                "Items",
                "Components",
                "Data",
                "Consumables",
            ...
            for typ in odyssey_plural_microresource_types:
                set_data.extend(
                    [
                        {
                            "itemName": thing["Name"],
                            "itemCount": thing["Count"],
                            "itemType": typ,
                        }
                        for thing in entry[typ]
                    ]
                )
                ...
                "suitLoadout": [
                    {
                        ...
                        "engineering": [
                            {"blueprintName": mod} for mod in x["WeaponMods"]
                        ],
                    }
                ...
                "suitLoadout": [
                    {
                        ...
                        "engineering": [
                            {"blueprintName": mod} for mod in entry["WeaponMods"]
                        ],
                    }
                ],
            }
        ...
        if event_name == "CommunityGoal":
            ...
            this.filter_events(
                current_credentials,
                lambda e: e.name
                not in ("setCommunityGoal", "setCommanderCommunityGoalProgress"),
            )
            ...
            for goal in entry["CurrentGoals"]:
                ...
                new_add_event(
                    "setCommanderCommunityGoalProgress", entry["timestamp"], data
                )
...
def cmdr_data(data: CAPIData, is_beta):
    ...
    if not this.station_marketid:
        this.station_marketid = (
            data["commander"]["docked"] and data["lastStarport"]["id"]
        )
...
def new_add_event(
    ...
    key = Credentials(
        str(cmdr), str(fid), api_key
    )  # this fails type checking due to `this` weirdness, hence str()
...
def clean_event_list(event_list: List[Event]) -> List[Event]:
    ...
    for event in event_list:
        is_bad, new_event = killswitch.check_killswitch(
            f"plugins.inara.worker.{event.name}", event.data, logger
        )
        if is_bad:
            continue
...
def new_worker():
    ...
    while True:
        ...
        if disabled_killswitch.disabled:
            logger.warning(
                f"Inara worker disabled via killswitch. ({disabled_killswitch.reason})"
            )
            continue
...
def send_data(url: str, data: Mapping[str, Any]) -> bool:
    ...
    response = this.session.post(
        url, data=json.dumps(data, separators=(",", ":")), timeout=_TIMEOUT
    )
...
def handle_api_error(
    data: Mapping[str, Any], status: int, reply: Dict[str, Any]
) -> None:
    """
...
def handle_individual_error(
    data_event: Dict[str, Any], reply_status: int, reply_text: str
) -> None:
    """
...
def handle_special_events(
    data_event: Dict[str, Any], reply_event: Dict[str, Any]
) -> None:
    """
