...
def system_url(system_name: str) -> str:
    ...
    if this.system_address:
        return requests.utils.requote_uri(
            f"https://inara.cz/galaxy-starsystem/?search={this.system_address}"
        )

    elif system_name:
        return requests.utils.requote_uri(
            f"https://inara.cz/galaxy-starsystem/?search={system_name}"
        )
...
def station_url(system_name: str, station_name: str) -> str:
    ...
    if this.system and this.station:
        return requests.utils.requote_uri(
            f"https://inara.cz/galaxy-station/?search={this.system}%20[{this.station}]"
        )
...
def journal_entry(  # noqa: C901, CCR001
    ...
    this.station_marketid = (
        entry.get("MarketID", this.station_marketid) or this.station_marketid
    )
    ...
    if (
        ...
        try:
            ...
            # Promotions
                ...
                new_add_event(
                    "setCommanderRankEngineer", entry["timestamp"], to_send_dict
                )
...
def clean_event_list(event_list: List[Event]) -> List[Event]:
    ...
    for e in event_list:
        bad, new_event = killswitch.check_killswitch(
            f"plugins.inara.worker.{e.name}", e.data, logger
        )
        if bad:
            continue
    ...
    if status // 100 != 2:  # 2xx == OK (maybe with warnings)
        ...
        plug.show_error(
            _("Error: Inara {MSG}").format(
                MSG=reply["header"].get("eventStatusText", status)
            )
        )
        ...
        for data_event, reply_event in zip(data["events"], reply["events"]):
            if reply_event["eventStatus"] != 200:
                if (
                    ...
                    logger.warning(
                        f'Inara\t{status} {reply_event.get("eventStatusText", "")}'
                    )
