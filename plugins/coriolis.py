...
def plugin_prefs(parent: tk.Widget, cmdr: str, is_beta: bool) -> tk.Frame:
    ...
    nb.Entry(conf_frame, textvariable=normal_textvar).grid(
    ...
    nb.Button(
        conf_frame,
        text=_("Reset"),
        command=lambda: normal_textvar.set(value=DEFAULT_NORMAL_URL),
    ...
    nb.Entry(conf_frame, textvariable=beta_textvar).grid(
        sticky=tk.EW, row=cur_row, column=1, padx=PADX
    )
    # LANG: Generic 'Reset' button label
    nb.Button(
        conf_frame,
        text=_("Reset"),
        command=lambda: beta_textvar.set(value=DEFAULT_BETA_URL),
    ).grid(sticky=tk.W, row=cur_row, column=2, padx=PADX)
    cur_row += 1
...
def _get_target_url(is_beta: bool) -> str:
    ...
    if override_mode not in ("auto", "normal", "beta"):
        ...
        logger.warning(
            f"Unexpected override mode {override_mode!r}! defaulting to auto!"
        )
        override_mode = "auto"
