...
sys.path: {sys.path}"""
        ...
        if args.j:
            ...
            try:
                ...
                logfiles = sorted(
                    (x for x in os.listdir(monitor.currentdir) if JOURNAL_RE.search(x)),
        ...
        if not deep_get(data, "commander", "name", default="").strip():
            ...
            data["lastSystem"]["name"] != monitor.system
            or (
        ...
        if (
            ...
            if (
                ...
                and deep_get(new_data, "lastSystem", "name") == monitor.system
        ...
        if args.n:
            try:
                ...
                eddn_sender.export_commodities(
                    data, monitor.is_beta, monitor.state["Odyssey"]
                )
                eddn_sender.export_outfitting(
                    data, monitor.is_beta, monitor.state["Odyssey"]
                )
                eddn_sender.export_shipyard(
                    data, monitor.is_beta, monitor.state["Odyssey"]
                )
