...
class _Translations:
    ...
    def install_dummy(self) -> None:
        ...
        builtins.__dict__["_"] = (
            lambda x: str(x).replace(r"\"", '"').replace("{CR}", "\n")
        )
    ...
    def install(self, lang: Optional[str] = None) -> None:  # noqa: CCR001
        ...
        for plugin in os.listdir(config.plugin_dir_path):
            ...
            if isdir(plugin_path):
                try:
                    self.translations[plugin] = self.contents(
                        cast(str, lang), str(plugin_path)
                    )

                except UnicodeDecodeError as e:
                    logger.warning(
                        f"Malformed file {lang}.strings in plugin {plugin}: {e}"
                    )

                except Exception:
                    logger.exception(
                        f"Exception occurred while parsing {lang}.strings in plugin {plugin}"
                    )
    ...
    def contents(self, lang: str, plugin_path: Optional[str] = None) -> Dict[str, str]:
        ...
        if translations.get(LANGUAGE_ID, LANGUAGE_ID) == LANGUAGE_ID:
            translations[LANGUAGE_ID] = str(
                lang
            )  # Replace language name with code if missing
    ...
    def translate(self, x: str, context: Optional[str] = None) -> str:
        ...
        if context:
            ...
            context = context[len(config.plugin_dir) + 1 :].split(os.sep)[  # noqa: E203
                0
            ]
        ...
        return self.translations[None].get(x) or str(x).replace(r"\"", '"').replace(
            "{CR}", "\n"
        )
    ...
    def available_names(self) -> Dict[Optional[str], str]:
        ...
        names.update(
            sorted(
                [
                    (lang, self.contents(lang).get(LANGUAGE_ID, lang))
    ...
    def respath(self) -> pathlib.Path:
        ...
        if getattr(sys, "frozen", False):
            if sys.platform == "darwin":
                return (
                    pathlib.Path(sys.executable).parents[0] / os.pardir / "Resources"
                ).resolve()
    ...
    def file(self, lang: str, plugin_path: Optional[str] = None) -> Optional[TextIO]:
        ...
        if plugin_path:
            ...
            return (self.respath() / f"{lang}.lproj" / "Localizable.strings").open(
                encoding="utf-16"
            )
...
class _Locale:
    ...
    def stringFromNumber(  # noqa: N802
        self, number: Union[float, int], decimals: Optional[int] = None
    ) -> str:
...
if __name__ == "__main__":
    regexp = re.compile(
        r"""_\([ur]?(['"])(((?<!\\)\\\1|.)+?)\1\)[^#]*(#.+)?"""
    )  # match a single line python literal
    seen: Dict[str, str] = {}
    python_files = sorted(x for x in os.listdir(".") if x.endswith(".py")) + sorted(
        join("plugins", x)
        for x in (os.listdir("plugins") if isdir("plugins") else [])
        if x.endswith(".py")
    )
    for f in python_files:
        with open(f, encoding="utf-8") as h:
            ...
            for line in h:
                ...
                if match and not seen.get(
                    match.group(2)
                ):  # only record first commented instance of a string
                    seen[match.group(2)] = (
                        match.group(4) and (match.group(4)[1:].strip()) + ". " or ""
                    ) + f"[{basename(f)}]"
    if seen:
