...
def extract_comments(
    ...
    if current_comment is None:
        print(
            f"No comment for {file}:{call.lineno} {lines[current_line_number]}",
            file=sys.stderr,
        )
        return None
...
def scan_directory(
    path: pathlib.Path, skip: Optional[List[pathlib.Path]] = None
...
class LangEntry:
    ...
    def files(self) -> str:
        ...
        file_locations = [
            ...
            + (
                f":{loc.line_end}"
                if loc.line_end is not None and loc.line_end != loc.line_start
                else ""
            )
...
def compare_lang_with_template(
    template: set[str], res: dict[pathlib.Path, list[ast.Call]]
) -> None:
    """
...
if __name__ == "__main__":
    ...
    if args.compare_lang:
        ...
        for path, calls in res.items():
            ...
            for c in calls:
                print(
                    f"    {c.lineno:4d}({c.col_offset:3d}):{c.end_lineno:4d}({c.end_col_offset:3d})\t",
                    ast.unparse(c),
                )
            print()
