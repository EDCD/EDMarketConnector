            ...
            if logfile:
                ...
                for line in loghandle:
                    ...
                    if match:
                        ...
                        # New Match for 1.6/2.1
                        # {<TIME>} System:"<SYSTEM_NAME>" StarPos:(<X>,<Y>,<Z>)ly(.*)
                        match = re.match('{(.*)} System:"(.*)" StarPos:\((.*),(.*),(.*)\)ly(.*)', line)
                        
                        if match:
                            visited     = match.group(1)
                            system      = match.group(2)
                            coordinates = [match.group(3), match.group(4), match.group(5)]
