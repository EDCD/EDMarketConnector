...
class PrefsVersion:
    ...
    def stringToSerial(
        self, versionStr: str
    ) -> int:  # noqa: N802, N803 # used in plugins
        """
    ...
    def shouldSetDefaults(
        self, addedAfter: str, oldTest: bool = True
    ) -> bool:  # noqa: N802,N803 # used in plugins
        """
...
if sys.platform == "darwin":
    ...
    try:
        ...
        objc.loadBundleVariables(
            HIServices, globals(), [("kAXTrustedCheckOptionPrompt", "@^{__CFString=}")]
        )
    ...
    BrowseCallbackProc = ctypes.WINFUNCTYPE(
        ctypes.c_int, HWND, ctypes.c_uint, LPARAM, LPARAM
    )
    ...
    if not is_wine:
        try:
            CalculatePopupWindowPosition = (
                ctypes.windll.user32.CalculatePopupWindowPosition
            )
...
class PreferencesDialog(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Tk, callback: Optional[Callable]):
        ...
        self.cmdrchanged_alarm: Optional[
            str
        ] = None  # This stores an ID that can be used to cancel a scheduled call
        ...
        notebook.bind(
            "<<NotebookTabChanged>>", self.tabchanged
        ...
        if sys.platform == "darwin":
            self.protocol(
                "WM_DELETE_WINDOW", self.apply
            )  # close button applies changes
        ...
        self.parent.wm_attributes(
            "-topmost", 0
        )  # needed for dialog to appear ontop of parent on OSX & Linux
    ...
    def __setup_output_tab(self, root_notebook: nb.Notebook) -> None:
        ...
        if prefsVersion.shouldSetDefaults(
            "0.0.0.0", not bool(config.get_int("output"))
        ):
        ...
        self.out_label = nb.Label(
            output_frame, text=_("Please choose what data to save")
        )
        ...
        self.out_csv_button.grid(
            columnspan=2, padx=self.BUTTONX, sticky=tk.W, row=row.get()
        )
        ...
        self.out_td_button = nb.Checkbutton(
            output_frame,
            text=_(
                "Market data in Trade Dangerous format file"
            ),  # LANG: Settings > Output option
            variable=self.out_td,
            command=self.outvarchanged,
        )
        self.out_td_button.grid(
            columnspan=2, padx=self.BUTTONX, sticky=tk.W, row=row.get()
        )
        ...
        self.out_ship_button.grid(
            columnspan=2, padx=self.BUTTONX, pady=(5, 0), sticky=tk.W, row=row.get()
        )
        self.out_auto = tk.IntVar(
            value=0 if output & config.OUT_MKT_MANUAL else 1
        )  # inverted
        ...
        self.outdir_label = nb.Label(
            output_frame, text=_("File location") + ":"
        ...
        if sys.platform == "darwin":
            text = _(
                "Change..."
            )  # LANG: macOS Preferences - files location selection button

        else:
            text = _(
                "Browse..."
            )  # LANG: NOT-macOS Settings - files location selection button
        ...
        self.outbutton.grid(
            column=1, padx=self.PADX, pady=self.PADY, sticky=tk.NSEW, row=row.get()
        )

        nb.Frame(output_frame).grid(
            row=row.get()
        )  # bottom spacer # TODO: does nothing?
    ...
    def __setup_config_tab(self, notebook: Notebook) -> None:  # noqa: CCR001
        ...
        default = (
            ...
            if config.default_journal_dir_path is not None
            else ""
        )
        ...
        if sys.platform == "darwin":
            ...
            text = _(
                "Browse..."
            )  # LANG: NOT-macOS Setting - files location selection button

        self.logbutton = nb.Button(
            config_frame,
            text=text,
            # LANG: Settings > Configuration - Label for Journal files location
            command=lambda: self.filebrowse(
                _("E:D journal file location"), self.logdir
            ),
        )
        self.logbutton.grid(
            column=3, padx=self.PADX, pady=self.PADY, sticky=tk.EW, row=row.get()
        )
        ...
        if sys.platform in ("darwin", "win32"):
            ...
            nb.Label(
                config_frame,
                text=_("Keyboard shortcut")
                if sys.platform  # LANG: Hotkey/Shortcut settings prompt on OSX
                == "darwin"
                else _("Hotkey"),  # LANG: Hotkey/Shortcut settings prompt on Windows
            ...
            if sys.platform == "darwin" and not was_accessible_at_launch:
                if AXIsProcessTrusted():
                    ...
                    nb.Button(
                        ...
                        command=self.enableshortcuts,
                    ).grid(padx=self.PADX, sticky=tk.E, row=row.get())

            else:
                self.hotkey_text = nb.Entry(
                    config_frame,
                    width=(20 if sys.platform == "darwin" else 30),
                    justify=tk.CENTER,
                )
                ...
                self.hotkey_text.grid(
                    column=1, columnspan=2, pady=(5, 0), sticky=tk.W, row=row.get()
                )
        ...
        self.disable_autoappupdatecheckingame = tk.IntVar(
            value=config.get_int("disable_autoappupdatecheckingame")
        )
        ...
        with row as cur_row:
            ...
            self.shipyard_provider = tk.StringVar(
                value=str(
                    shipyard_provider
                    if shipyard_provider in plug.provides("shipyard_url")
                    else "EDSY"
                )
            )
            ...
            nb.Label(config_frame, text=_("Shipyard")).grid(
            ...
            self.alt_shipyard_open = tk.IntVar(
                value=config.get_int("use_alt_shipyard_open")
            )
        ...
        with row as cur_row:
            ...
            self.system_provider = tk.StringVar(
                value=str(
                    system_provider
                    if system_provider in plug.provides("system_url")
                    else "EDSM"
                )
            )

            # LANG: Configuration - Label for selection of 'System' provider website
            nb.Label(config_frame, text=_("System")).grid(
                padx=self.PADX, pady=2 * self.PADY, sticky=tk.W, row=cur_row
            )
        ...
        with row as cur_row:
            ...
            self.station_provider = tk.StringVar(
                value=str(
                    station_provider
                    if station_provider in plug.provides("station_url")
                    else "eddb"
                )
            )

            # LANG: Configuration - Label for selection of 'Station' provider website
            nb.Label(config_frame, text=_("Station")).grid(
                padx=self.PADX, pady=2 * self.PADY, sticky=tk.W, row=cur_row
            )
    ...
    def __setup_privacy_tab(self, notebook: Notebook) -> None:
        ...
        self.hide_multicrew_captain = tk.BooleanVar(
            value=config.get_bool("hide_multicrew_captain", default=False)
        )
        self.hide_private_group = tk.BooleanVar(
            value=config.get_bool("hide_private_group", default=False)
        )
        row = AutoInc()
    ...
    def __setup_appearance_tab(self, notebook: Notebook) -> None:
        ...
        self.lang = tk.StringVar(
            value=self.languages.get(config.get_str("language"), _("Default"))
        )
        self.always_ontop = tk.BooleanVar(value=bool(config.get_int("always_ontop")))
        self.minimize_system_tray = tk.BooleanVar(
            value=config.get_bool("minimize_system_tray")
        )
        self.theme = tk.IntVar(value=config.get_int("theme"))
        self.theme_colors = [
            config.get_str("dark_text"),
            config.get_str("dark_highlight"),
        ]
        self.theme_prompts = [
        ...
        with row as cur_row:
            ...
            nb.Label(appearance_frame, text=_("Language")).grid(
                padx=self.PADX, sticky=tk.W, row=cur_row
            )
            self.lang_button = nb.OptionMenu(
                appearance_frame, self.lang, self.lang.get(), *self.languages.values()
            )
            self.lang_button.grid(
                column=1, columnspan=2, padx=self.PADX, sticky=tk.W, row=cur_row
            )
        ...
        nb.Label(appearance_frame, text=_("Theme")).grid(
            columnspan=3, padx=self.PADX, sticky=tk.W, row=row.get()
        )
        ...
        with row as cur_row:
            ...
            self.theme_button_0.grid(
                column=1, padx=self.PADX, pady=self.PADY, sticky=tk.NSEW, row=cur_row
            )
        ...
        with row as cur_row:
            ...
            self.ui_scaling_defaultis = nb.Label(
                ...
                text=_(
                    "100 means Default{CR}Restart Required for{CR}changes to take effect!"
                ),
            ).grid(
                column=3, padx=self.PADX, pady=2 * self.PADY, sticky=tk.E, row=cur_row
            )
        ...
        with row as cur_row:
            ...
            self.transparency.set(
                config.get_int("ui_transparency") or 100
            )  # Default to 100 for users
            self.transparency_bar = tk.Scale(
                ...
                tickinterval=10,
                resolution=5,
                command=lambda _: self.parent.wm_attributes(
            ...
            nb.Label(
                ...
                text=_(
                    "100 means fully opaque.{CR}" "Window is updated in real time"
                ).format(CR="\n"),
            ).grid(
                column=3, padx=self.PADX, pady=self.PADY * 2, sticky=tk.E, row=cur_row
            )
    ...
    def __setup_plugin_tab(self, notebook: Notebook) -> None:  # noqa: CCR001
        ...
        nb.Label(
            ...
            text=_(
                "Tip: You can disable a plugin by{CR}adding '{EXT}' to its folder name"
            ).format(EXT=".disabled"),
        ...
        if len(enabled_plugins):
            ...
            for plugin in enabled_plugins:
                if plugin.name == plugin.folder:
                    ...
                    label = nb.Label(
                        plugins_frame, text=f"{plugin.folder} ({plugin.name})"
                    )
        ...
        if len(plug.PLUGINS_not_py3):
            ...
            nb.Label(
                plugins_frame, text=_("Plugins Without Python 3.x Support:") + ":"
            ).grid(padx=self.PADX, sticky=tk.W)

            for plugin in plug.PLUGINS_not_py3:
                if (
                    plugin.folder
                ):  # 'system' ones have this set to None to suppress listing in Plugins prefs tab
                    nb.Label(plugins_frame, text=plugin.name).grid(
                        columnspan=2, padx=self.PADX * 2, sticky=tk.W
                    )
        ...
        disabled_plugins = list(
            filter(lambda x: x.folder and not x.module, plug.PLUGINS)
        )
    ...
    def filebrowse(self, title, pathvar):
        ...
        directory = None
        if sys.platform == "win32" and current_locale[1] not in (
            "utf8",
            "UTF8",
            "utf-8",
            "UTF-8",
            ...
            def browsecallback(
                hwnd, uMsg, lParam, lpData
            ):  # noqa: N803 # Windows API convention
                # set initial folder
                if uMsg == BFFM_INITIALIZED and lpData:
                    ctypes.windll.user32.SendMessageW(
                        hwnd, BFFM_SETSELECTION, 1, lpData
                    )
                return 0
            ...
            browseInfo.lParam = (
                pathvar.get().startswith("~")
                and join(config.home_path, pathvar.get()[2:])
                or pathvar.get()
            )
    ...
    def disable_autoappupdatecheckingame_changed(self) -> None:
        ...
        config.set(
            "disable_autoappupdatecheckingame",
            self.disable_autoappupdatecheckingame.get(),
        )
    ...
    def themevarchanged(self) -> None:
        ...
        (
            self.theme_button_0["foreground"],
    ...
    def apply(self) -> None:
        ...
        config.set(
            ...
            + (
                ...
                & (config.OUT_MKT_EDDN | config.OUT_SYS_EDDN | config.OUT_SYS_DELAY)
            ),
        )

        config.set(
            "outdir",
            join(config.home_path, self.outdir.get()[2:])
            if self.outdir.get().startswith("~")
            else self.outdir.get(),
        )

        logdir = self.logdir.get()
        if (
            config.default_journal_dir_path
    ...
    def _destroy(self) -> None:
        ...
        self.parent.wm_attributes(
            "-topmost", 1 if config.get_int("always_ontop") else 0
        )
        self.destroy()
