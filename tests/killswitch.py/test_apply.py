...
def test_apply(
    source: UPDATABLE_DATA, key: str, action: str, to_set: Any, result: UPDATABLE_DATA
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"multilevel.keys": {"with.dots": False}},
            "multilevel.keys.with.dots",
            "",
            True,
            {"multilevel.keys": {"with.dots": True}},
        ),
        (
            {"dotted.key.one.level": False},
            "dotted.key.one.level",
            "",
            True,
            {"dotted.key.one.level": True},
        ),
    ],
)
def test_deep_get(
    ...
    cpy = copy.deepcopy(source)
    killswitch._deep_apply(
        target=cpy, path=key, to_set=to_set, delete=action == "delete"
    )
    assert cpy == result
