    ...
    [
        killswitch.KillSwitches(
            ...
            kills={
                ...
                "delete-action": killswitch.SingleKill(
                    "delete-action", "remove stuff", delete_fields=["a", "b.c"]
                ),
                "delete-action-l": killswitch.SingleKill(
                    "delete-action-l", "remove stuff", delete_fields=["2", "0"]
                ),
                "set-action": killswitch.SingleKill(
                    "set-action", "set stuff", set_fields={"a": False, "b.c": True}
                ),
                "redact-action": killswitch.SingleKill(
                    "redact-action", "redact stuff", redact_fields=["a", "b.c"]
                ),
            },
        )
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            "1.0.0",
        ),  # set should be thrown out because it cant be indext
        (
            ["a", "b"],
            "delete-action-l",
            True,
            ["b"],
            "1.0.0",
        ),  # has a missing value, but that's fine for delete
        (["a", "b"], "delete-action-l", True, ["a", "b"], "1.1.0"),  # wrong version
    ],
)
def test_killswitch(
    input: killswitch.UPDATABLE_DATA,
    kill: str,
    should_pass: bool,
    result: killswitch.UPDATABLE_DATA | None,
    version: str,
) -> None:
    ...
    assert (
        not should_return
    ) == should_pass, f'expected to {"pass" if should_pass else "fail"}, but {"passed" if not should_pass else "failed"}'
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"set_fields": {"test": None}, "redact_fields": ["test"]},
            {},
            {"test": "REDACTED"},
        ),
        (
            {
                "set_fields": {"test": None},
                "redact_fields": ["test"],
                "delete_fields": ["test"],
            },
            {},
...
def test_operator_precedence(
    kill_dict: killswitch.SingleKillSwitchJSON,
    input: killswitch.UPDATABLE_DATA,
    result: killswitch.UPDATABLE_DATA,
) -> None:
    """Ensure that operators are being applied in the correct order."""
    kill = killswitch.SingleKill(
        "",
        "",
        kill_dict.get("redact_fields"),
        kill_dict.get("delete_fields"),
        kill_dict.get("set_fields"),
    )
...
def test_check_multiple(
    ...
    should_return, data = TEST_SET.check_multiple_killswitches(
        input, *names, version="1.0.0"
    )
