...
def _get_fuzz(_type: Any, num_values=50, value_length=(0, 10)) -> list:
    return [
        _fuzz_generators[_type](random.randint(*value_length))
...
def _build_test_list(
    static_data, random_data, random_id_name="random_test_{i}"
) -> Iterable:
    return itertools.chain(
        static_data, _make_params(random_data, id_name=random_id_name)
    )
...
class TestNewConfig:
    ...
    @mark.parametrize(
        "i",
        _build_test_list(int_tests, _get_fuzz(int, value_length=(-big_int, big_int))),
    )
    ...
    @mark.parametrize(
        "string", _build_test_list(string_tests, _get_fuzz(str, value_length=(0, 512)))
    )
...
class TestOldNewConfig:
    ...
    @mark.parametrize(
        "i", _build_test_list(int_tests, _get_fuzz(int, 50, (-big_int, big_int)))
    )
    ...
    def test_string(self, string: str) -> None:
        ...
        string = string.replace(
            "\r", ""
        )  # The old config does _not_ support \r in its entries. We do.
    ...
    def test_list(self, lst: List[str]) -> None:
        ...
        lst = [
            x.replace("\r", "") for x in lst
        ]  # OldConfig on linux fails to store these correctly
        if sys.platform == "win32":
    ...
    @mark.skipif(
        sys.platform == "win32", reason="Old Windows config does not support bool types"
    )
    @mark.parametrize("b", bool_tests)
    def test_bool(self, b: bool) -> None:
