...
class Plugin:
    ...
    def __init__(
        self,
        name: str,
        loadfile: Optional[str],
        plugin_logger: Optional[logging.Logger],
    ):
        """
        ...
        self.folder: Optional[
            str
        ] = name  # basename of plugin folder. None for internal plugins.
        self.module = None  # None for disabled plugins.
        self.logger: Optional[logging.Logger] = plugin_logger

        if loadfile:
            self._load_plugin(loadfile)
        else:
            self._disable_plugin()

    def _load_plugin(self, loadfile: str) -> None:
        logger.info(f'loading plugin "{self.name.replace(".", "_")}" from "{loadfile}"')
        try:
            self._load_module(loadfile)
        except Exception:
    ...
    def _load_module(self, loadfile: str) -> None:
        try:
            ...
            if self._has_plugin_start3():
                self._set_plugin_name(loadfile)

            elif self._has_plugin_start():
                logger.warning(f"plugin {self.name} needs migrating\n")
                PLUGINS_not_py3.append(self)

            else:
                logger.error(f"plugin {self.name} has no plugin_start3() function")
        except Exception:
            logger.exception(f': Failed for Plugin "{self.name}"')
            raise

    def _encode_plugin_name(self) -> str:
        return (
            self.name.encode(encoding="ascii", errors="replace")
            .decode("utf-8")
            .replace(".", "_")
        )

    def _has_plugin_start3(self) -> bool:
        return hasattr(self.module, "plugin_start3")

    def _has_plugin_start(self) -> bool:
    ...
    def get_prefs(
        self, parent: ttk.Notebook, cmdr: Optional[str], is_beta: bool
    ) -> Optional[nb.Frame]:
        """
        ...
        :returns: a myNotebook Frame or None
        """
...
def load_plugins(master: tk.Tk) -> None:  # noqa: CCR001
    ...
    PLUGINS.clear()  # Clear the existing plugins list

    # Load internal plugins
    internal_plugins = []
    internal_plugin_dir = config.internal_plugin_dir_path
    for name in sorted(os.listdir(internal_plugin_dir)):
        if name.endswith(".py") and name[0] not in [".", "_"]:
            try:
                plugin = Plugin(
                    name[:-3],
                    os.path.join(internal_plugin_dir, name),
                    logger,
                )
                plugin.folder = None  # Suppress listing in Plugins prefs tab
                internal_plugins.append(plugin)
            except Exception:
                logger.exception(f'Failure loading internal Plugin "{name}"')

    PLUGINS.extend(sorted(internal_plugins, key=lambda p: p.name.lower()))
    ...
    for name in sorted(os.listdir(plugin_dir)):
        if os.path.isdir(os.path.join(plugin_dir, name)) and name[0] not in [".", "_"]:
            try:
                plugin_path = os.path.join(plugin_dir, name, "load.py")
                if name.endswith(".disabled"):
                    name = name.rsplit(".", 1)[0]
                    external_plugins.append(Plugin(name, None, logger))
                else:
                    # Add plugin's folder to load path in case plugin has internal package dependencies
                    sys.path.append(os.path.join(plugin_dir, name))
                    ...
                    external_plugins.append(Plugin(name, plugin_path, plugin_logger))
            except Exception:
                logger.exception(f'Failure loading found Plugin "{name}"')

    PLUGINS.extend(sorted(external_plugins, key=lambda p: p.name.lower()))
...
def provides(fn_name: str) -> List[str]:
    ...
    .. versionadded:: 3.0.2
    """
    return [plugin.name for plugin in PLUGINS if plugin._get_func(fn_name)]


def _invoke_plugins(
    plugin_type: str, fn_name: str, *args: Any, fallback: Optional[str] = None
) -> Optional[str]:
...
def invoke(
    ...
    .. versionadded:: 3.0.2
    """
    return _invoke_plugins(plugin_name, fn_name, *args, fallback=fallback)
...
def _notify_prefs_plugins(fn_name: str, cmdr: Optional[str], is_beta: bool) -> None:
    for plugin in PLUGINS:
...
def notify_journal_entry_cqc(
    cmdr: str, is_beta: bool, entry: MutableMapping[str, Any], state: Mapping[str, Any]
) -> Optional[str]:
    """
...
def notify_dashboard_entry(
    cmdr: str, is_beta: bool, entry: MutableMapping[str, Any]
) -> Optional[str]:
    """
