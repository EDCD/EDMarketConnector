    ...
    def get_cfbundle_localizations() -> Set:
        ...
        return sorted(
            (
                [
                    x[: -len(".lproj")]
                    for x in os.listdir(join(SPARKLE, "Resources"))
                    if x.endswith(".lproj")
                ]
            )
            | (
...
if sys.platform == "darwin":
    if isdir(f"{dist_dir}/{applongname}.app"):  # from CFBundleName
        ...
        for x in os.listdir("L10n"):
            if x.endswith(".strings"):
                ...
                os.mkdir(path)
                codecs.open(f"{path}/Localizable.strings", "w", "utf-16").write(
                    codecs.open(f"L10n/{x}", "r", "utf-8").read()
                )

        if macdeveloperid:
            os.system(
                f'codesign --deep -v -s "Developer ID Application: {macdeveloperid}" {dist_dir}/{appname}.app'
            )

        # Make zip for distribution, preserving signature
        package_filename = f"{appname}_mac_{appversion_nobuild()}.zip"
        os.system(
            f"cd {dist_dir}; ditto -ck --keepParent --sequesterRsrc {appname}.app ../{package_filename}; cd .."
        )
    ...
    os.system(
        rf'"{WIXPATH}\heat.exe" dir {dist_dir}\ -ag -sfrag -srid -suid -out {components_file}'
    )
    ...
    os.system(
        rf'"{WIXPATH}\light.exe" -b {dist_dir}\ -sacl -spdb -sw1076 {appname}.wixobj -out {package_filename}'
    )
    ...
    lcids = [
        ...
        for x in re.search(  # type: ignore
            r'Languages\s*=\s*"(.+?)"', open(f"{appname}.wxs").read()
        )
        .group(1)
        .split(",")
    ]
    ...
    for lcid in lcids[1:]:
        ...
        os.system(
            rf'cscript /nologo "{SDKPATH}\WiLangId.vbs" {gettempdir()}\{appname}_{lcid}.msi Product {lcid}'
        )
        os.system(
            rf'"{SDKPATH}\MsiTran.Exe" -g {gettempdir()}\{appname}_1033.msi {gettempdir()}\{appname}_{lcid}.msi {gettempdir()}\{lcid}.mst'
        )  # noqa: E501 # Not going to get shorter
        os.system(
            rf'cscript /nologo "{SDKPATH}\WiSubStg.vbs" {package_filename} {gettempdir()}\{lcid}.mst {lcid}'
        )
