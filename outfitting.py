...
def lookup(module, ship_map, entitled=False) -> Optional[dict]:  # noqa: C901, CCR001
    ...
    if not moduledata:
        moduledata.update(
            pickle.load(open(join(config.respath_path, "modules.p"), "rb"))
        )
    ...
    # Hardpoints - e.g. Hpt_Slugshot_Fixed_Medium
        ...
        if len(name) > 4:
            if (
                name[4] in weaponoldvariant_map
    ...
    for thing in [
        ...
        "class",
        "rating",
    ]:  # Don't consider mass etc as mandatory
