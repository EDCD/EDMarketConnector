...
class _Theme(object):
    ...
    def register(self, widget):
        ...
        if widget not in self.widgets:
            ...
            if isinstance(widget, tk.BitmapImage):
                ...
                or isinstance(widget, tk.Canvas)
            ):
                if (
                ...
                if "foreground" in widget.keys() and widget["foreground"] not in [
                    "",
                    self.defaults["fg"],
                ]:
                    attribs.add("fg")
                if "background" in widget.keys() and widget["background"] not in [
                    "",
                    self.defaults["bg"],
                ]:
                    attribs.add("bg")
    ...
    def _enter(self, event, image):
        ...
        if widget and widget["state"] != tk.DISABLED:
            ...
            if image:
                image.configure(
                    foreground=self.current["activeforeground"],
    ...
    def _colors(self, root, theme):
        ...
        if theme:
            ...
            self.current = {
                ...
                "disabledforeground": "#%02x%02x%02x"
                % (int(r / 384), int(g / 384), int(b / 384)),
                ...
                "font": (
                    theme > 1
                    and not 0x250 < ord(_("Cmdr")[0]) < 0x3000
                    and tkFont.Font(family="Euro Caps", size=10, weight=tkFont.NORMAL)
                    or "TkDefaultFont"
                ),
            }
        else:
            ...
            self.current = {
                "background": (
                    sys.platform == "darwin"
                    and "systemMovableModalBackground"
                ...
                "activebackground": (
                    sys.platform == "win32"
                    and "SystemHighlight"
                    or style.lookup("TLabel", "background", ["active"])
                ),
                "activeforeground": (
                    sys.platform == "win32"
                    and "SystemHighlightText"
                    or style.lookup("TLabel", "foreground", ["active"])
                ),
                "disabledforeground": style.lookup(
                    "TLabel", "foreground", ["disabled"]
                ),
                "highlight": "blue",
                "font": "TkDefaultFont",
            }
    ...
    def _update_widget(self, widget):
        assert widget in self.widgets, '%s %s "%s"' % (
            widget.winfo_class(),
            widget,
            "text" in widget.keys() and widget["text"],
        )
    ...
    def apply(self, root):
        ...
        if sys.platform == "darwin":
            ...
            appearance = NSAppearance.appearanceNamed_(
                theme and "NSAppearanceNameDarkAqua" or "NSAppearanceNameAqua"
            )
            ...
            SetWindowLongW(
                hwnd, GWL_STYLE, GetWindowLongW(hwnd, GWL_STYLE) & ~WS_MAXIMIZEBOX
            )  # disable maximize
            SetWindowLongW(
                hwnd,
                GWL_EXSTYLE,
                theme > 1 and WS_EX_APPWINDOW | WS_EX_LAYERED or WS_EX_APPWINDOW,
            )  # Add to taskbar
            root.deiconify()
            ...
            if dpy:
                ...
                XChangeProperty(
                    dpy,
                    parent,
                    motif_wm_hints_property,
                    motif_wm_hints_property,
                    32,
                    PropModeReplace,
                    theme and motif_wm_hints_dark or motif_wm_hints_normal,
                    5,
                )
                XFlush(dpy)
            else:
